...
- English is the default for a fresh install (`config.DEFAULT_LANGUAGE = "en"`); the choice is persisted in `config.json`'s `language` field via the main menu's `[4] Idioma / Language` option (`cli.py::_language_menu()`), which also calls `i18n.set_language()` immediately so the change is live without a restart.
