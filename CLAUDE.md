...
aws-sso-sync --update        # updates to the latest release tag and exits, no menu
...
- **`aws_sso_sync/config.py`** — the source of truth for tenants/accounts, replacing the old hardcoded dict. Persists to `~/.config/aws-sso-sync/config.json` (plain stdlib `json`, no dependency). `Tenant` holds `sso_region`/`sso_session`/`sso_start_url` + a list of `Account` (`label`, `role`, `account_id`, `sso_role_name`, `sso_profile`, `credentials_profile`). Starts empty on first run — nothing is pre-seeded. The same file also carries a top-level `language` field (`load_language()`/`save_language()`); `save()`/`save_language()` both round-trip through `_read_raw()` first so writing one never clobbers the other.
- **`aws_sso_sync/i18n/`** — see "Internationalization (i18n)" below.
...
- **`aws_sso_sync/cli.py`** — main menu (Sync / Maintenance / Language / About / Exit, labels shown in the active language); option 3 opens `_language_menu()`, which is deliberately **not** routed through `t()` (its own two lines are hardcoded bilingual "Idioma / Language" / "Opción / Option") so it stays readable even if the active language is the wrong one for the user reading it. Option 4 opens `_about()` — version (`__version__`), author, website (`AUTHOR_WEBSITE`), repo (`REPO_URL`), license (`LICENSE`), all as module-level constants at the top of `cli.py`; this is now the only place author/site info is shown — the farewell message on "Salir"/"Exit" is plain, no credit line. Self-update is **not** a menu option — it's the `--update` startup flag (`aws-sso-sync --update`), checked in `main()` right after `i18n.set_language()`/logging setup and *before* `check_aws_cli()` (an update doesn't need `aws` on `PATH` at all, and skipping the check lets `--update` fix a broken/outdated install before AWS CLI is even sorted out); it runs `_update()` then exits immediately, never opening the interactive menu. `_update()` fetches tags in `$AWS_SSO_SYNC_HOME` (set by the `install.sh`-generated launcher) and fast-forwards (`git merge --ff-only`) to the latest `vX.Y.Z` tag — deliberately *not* the tip of `master` — followed by an editable reinstall; see "Versioning & releases" below for why. Both subprocess calls run with `capture_output=True` — only a short "Actualizando..." / "✅ Actualización completada (vX.Y.Z)" (or the error text on failure) is printed; the raw `git`/`pip` output only goes to the `--logs-enabled` debug log, never the screen. Parses `--logs-enabled`/`--update` via `argparse` and calls `logging_setup.setup_logging()` before anything else. Each main-loop iteration is wrapped in `try/except KeyboardInterrupt` — nothing in `menu_login.py`/`menu_maintenance.py` catches it, so Ctrl+C at any prompt, however deep in a submenu, propagates straight up to this one handler and lands back at the main menu instead of crashing with a traceback. The startup `check_aws_cli()` call (before the loop even starts, and skipped entirely for `--update`) is wrapped separately, since it isn't covered by the loop's handler.
...
All user-facing menu text (every `print()`/`input()` prompt the user sees, not `logger.debug(...)` calls, which stay as internal Spanish diagnostics — see [[feedback_language_convention]]) goes through `i18n.t(key, **kwargs)`. There's no `gettext`/`.mo` compilation on purpose — that needs an external `msgfmt` binary, which would break the "stdlib only, no build step" guarantee. Instead:

- **`aws_sso_sync/i18n/__init__.py`** — `CATALOGS: dict[str, dict[str, str]]` (one entry per language code), `DEFAULT_LANGUAGE = "en"`, `set_language()`/`get_language()` (in-process, not persisted — that's `config.py`'s job), and `t(key, **kwargs)`, which looks up `key` in the active catalog, falls back to `DEFAULT_LANGUAGE`, and finally falls back to the raw `key` string so a missing translation degrades visibly instead of crashing.
- **`aws_sso_sync/i18n/catalog/en.py`** — the reference catalog; every key used anywhere in the code must exist here.
- **`aws_sso_sync/i18n/catalog/es.py`** — must contain the exact same key set as `en.py`.
- **`scripts/check_i18n.py`** — fails if any catalog's keys don't exactly match `en.py`'s; wired into `ci.yml`. Run it locally after touching a catalog.
- English is the default for a fresh install (`config.DEFAULT_LANGUAGE = "en"`); the choice is persisted in `config.json`'s `language` field via the main menu's `[3] Idioma / Language` option (`cli.py::_language_menu()`), which also calls `i18n.set_language()` immediately so the change is live without a restart.
- **Adding a language**: copy `en.py` to `aws_sso_sync/i18n/catalog/<code>.py`, translate every value (keep every key identical), register it in `i18n/__init__.py`'s `CATALOGS`/`LANGUAGE_NAMES` dicts, and run `scripts/check_i18n.py`. Full steps in [CONTRIBUTING.md](CONTRIBUTING.md#adding-a-language).
...
- **`.github/workflows/ci.yml`** — on every push/PR to `master`: `py_compile` syntax check, `ruff check`, `ruff format --check`, `scripts/check_i18n.py` (catalog key parity).
